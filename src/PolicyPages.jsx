...
/* ============================================================
   AUTHOR PAYOUT ACCOUNT RESPONSIBILITY POLICY
   ============================================================ */
export function PayoutResponsibilityPage({ go }) {
  return (
    <PolicyShell title="Author Payout Account Responsibility" eyebrow="Author Agreements" go={go}>
      <Sec>
        <Box color="#FFF6E8">
          <strong>Summary for authors:</strong> You are responsible for connecting, verifying, and maintaining your own payout account. Little Amour Books will hold your earnings until your account is fully set up. Your book may still be sold during this time.
        </Box>
      </Sec>

      <Sec title="1. Author Payout Account Obligation">
        <p>Author is responsible for creating, connecting, verifying, and maintaining an active payout account through Stripe or any other payment processor approved by Little Amour Books. Author must provide accurate payment, banking, tax, and identity verification information as required by the payment processor and applicable law.</p>
        <p>Little Amour Books is not responsible for delayed, failed, withheld, reversed, or undeliverable payments caused by:</p>
        <ul className="pol-list">
          <li>Author's failure to connect a payout account</li>
          ...
          <li>Suspected fraud or compliance review</li>
          <li>Author's failure to respond to payout setup requests</li>
        </ul>
        <Box color="#FFF6E8">
          If Author does not connect or maintain a valid payout account, Little Amour Books may hold Author earnings until Author completes payout setup. <strong>Held earnings will not accrue interest.</strong>
        </Box>
        <p>Author remains responsible for any taxes, reporting obligations, and fees related to Author's earnings, except where Little Amour Books is legally required to withhold, report, or remit amounts.</p>
      </Sec>

      <Sec title="2. Payout Setup Steps Required">
        <p>After project acceptance, authors must complete all of the following before earnings can be released:</p>
        <ol className="pol-list">
          <li><strong>Connect Stripe account</strong> — Connect your existing Stripe account or create one at stripe.com.</li>
          <li><strong>Complete identity verification</strong> — Provide government-issued ID as required by Stripe and applicable law.</li>
          <li><strong>Add payout method</strong> — Link a bank account, debit card, or other approved payout method.</li>
          <li><strong>Complete required tax information</strong> — Submit W-9 (US), W-8BEN (international), or equivalent tax form as required.</li>
          <li><strong>Confirm payout responsibility terms</strong> — Acknowledge these terms by checking the confirmation box in your author dashboard.</li>
        </ol>
      </Sec>

      <Sec title="3. Effect on Book Publication">
        <p>Failure to connect a payout account does <strong>not</strong> prevent Little Amour Books from listing, selling, or distributing an Author's published book. The book may continue to be sold after publication regardless of payout account status.</p>
        <Box color="#FFF6E8">
          Earnings generated during a payout hold period will be accumulated and held by Little Amour Books. Earnings will be released to the Author once a valid, verified payout account is established. Held earnings will not accrue interest.
        </Box>
      </Sec>

      <Sec title="4. Publication Checklist">
        <p>Before a book goes live, Little Amour Books confirms:</p>
        <ul className="pol-list">
          <li>Publishing Agreement signed</li>
          <li>Author payout account connected — <em>or</em> payout hold acknowledged in writing</li>
          <li>Tax information submitted or payout hold acknowledged</li>
          <li>Identity verification complete or pending with notification to author</li>
        </ul>
        <p>Books will not be blocked from publication solely due to incomplete payout setup, but earnings will be held until setup is complete.</p>
      </Sec>

      <Sec title="5. Author-Facing Notice">
        <Box color="#EAF0EB">
          <strong>Important:</strong> You can publish with Little Amour Books only after signing the Publishing Agreement. Earnings can only be paid after your payout account is connected, verified, and active. If your payout account is not set up, your earnings will be held — your book will still be available for sale.
        </Box>
      </Sec>

      <Sec title="6. Payout Hold Notifications">
        <p>If a payout hold is in effect, Little Amour Books will:</p>
        <ul className="pol-list">
          <li>Notify the Author by email at the address on file</li>
          <li>Provide clear instructions for completing payout setup</li>
          <li>Make reasonable follow-up attempts as required</li>
          <li>Record the notification date and held earnings amount in the Author's account</li>
        </ul>
        <p>It is the Author's responsibility to maintain a valid, monitored email address and to respond to payout setup requests in a timely manner.</p>
      </Sec>

      <Sec title="7. Legal Savings Clause">
        <Box color="#EAF0EB">
          Nothing in this section limits any rights Author may have under applicable employment, contractor, or income-payment laws. Where law requires payment within a specific timeframe, Little Amour Books will comply to the extent technically possible given payout account constraints.
        </Box>
      </Sec>

      <Sec title="Contact">
        <p>Questions about your payout account or earnings hold: <a href={`mailto:${SUPPORT_EMAIL}`} className="pol-link">{SUPPORT_EMAIL}</a></p>
      </Sec>
    </PolicyShell>
  );
}
...
/* ============================================================
   AUTHOR PAYOUT WARNING BANNER (author-facing)
   ============================================================ */
export function PayoutWarningBanner({ stripeConnected = false, go }) {
  if (stripeConnected) return null;
  return (
    <div className="payout-warning-banner">
      <span className="payout-warning-icon">⚠️</span>
      <div>
        <strong>Payout account not connected.</strong>{" "}
        You can publish with Little Amour Books only after signing the Publishing Agreement. Earnings can only be paid after your payout account is connected, verified, and active.{" "}
        <button className="pol-link-inline" onClick={() => go && go("payout-setup")}>Set up payout account →</button>
      </div>
    </div>
  );
}
...
/* ============================================================
   PAYOUT SETUP ONBOARDING FLOW (author-facing, after acceptance)
   ============================================================ */
const PAYOUT_STEPS = [
  { id: "connect",  label: "Connect Stripe",           icon: "🔗", desc: "Connect your Stripe account to receive payments. If you don't have one, you'll create one at stripe.com." },
  { id: "verify",   label: "Verify Identity",          icon: "🪪", desc: "Stripe requires government-issued ID to verify your identity as required by law." },
  { id: "payout",   label: "Add Payout Method",        icon: "🏦", desc: "Link a bank account or debit card to receive your earnings." },
  { id: "tax",      label: "Complete Tax Information",  icon: "📋", desc: "Submit your W-9 (US) or W-8BEN (international). Required by law before earnings can be released." },
  { id: "confirm",  label: "Confirm Payout Terms",     icon: "✅", desc: "Confirm you have read and accept the Payout Account Responsibility terms." },
];

export function PayoutOnboardingFlow({ author, onComplete, go }) {
  const [step, setStep] = React.useState(0);
  const [checked, setChecked] = React.useState({});
  const [confirmed, setConfirmed] = React.useState(false);
  const [done, setDone] = React.useState(false);

  const current = PAYOUT_STEPS[step];
  const allPrev = PAYOUT_STEPS.slice(0, step).every(s => checked[s.id]);

  const markDone = (id) => {
    const next = { ...checked, [id]: true };
    setChecked(next);
    if (step < PAYOUT_STEPS.length - 1) setStep(s => s + 1);
    else if (confirmed) { setDone(true); onComplete && onComplete(next); }
  };

  if (done) return (
    <div className="payout-done">
      <div style={{ fontSize: 52, marginBottom: 14 }}>🎉</div>
      <h2 style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 24, color: "#131A30", marginBottom: 10 }}>Payout account ready!</h2>
      <p style={{ color: "#5E5468", fontSize: 15, lineHeight: 1.65, marginBottom: 20 }}>Your earnings will be released to your connected account. Thank you for completing setup.</p>
      <button className="payout-btn-gold" onClick={() => go && go("dashboard")}>Go to your dashboard →</button>
    </div>
  );

  return (
    <div className="payout-flow">
      <style>{PAYOUT_CSS}</style>
      <div className="payout-flow-header">
        <h2 className="payout-h2">Set up your payout account</h2>
        <p className="payout-sub">Complete all steps to receive your earnings. Your book can be published before this is done — but earnings are held until setup is complete.</p>
      </div>

      {/* Warning banner */}
      <div className="payout-warning-banner" style={{ marginBottom: 24 }}>
        <span className="payout-warning-icon">⚠️</span>
        <span>You can publish with Little Amour Books only after signing the Publishing Agreement. <strong>Earnings can only be paid after your payout account is connected, verified, and active.</strong></span>
      </div>

      {/* Step indicators */}
      <div className="payout-steps">
        {PAYOUT_STEPS.map((s, i) => (
          <div key={s.id} className={"payout-step-ind" + (i === step ? " on" : checked[s.id] ? " done" : "")}>
            <span className="payout-step-dot">{checked[s.id] ? "✓" : i + 1}</span>
            <span className="payout-step-lbl">{s.label}</span>
          </div>
        ))}
      </div>

      {/* Current step */}
      <div className="payout-step-card">
        <div className="payout-step-icon">{current.icon}</div>
        <h3 className="payout-step-title">Step {step + 1}: {current.label}</h3>
        <p className="payout-step-desc">{current.desc}</p>

        {current.id === "connect" && (
          <div>
            <a href="https://connect.stripe.com/oauth/authorize?response_type=code&client_id=STRIPE_CLIENT_ID&scope=read_write" target="_blank" rel="noopener noreferrer" className="payout-btn-gold payout-stripe-btn">
              Connect with Stripe →
            </a>
            <p className="payout-step-hint">You'll be taken to Stripe to connect or create your account. Return here when done.</p>
          </div>
        )}
        {current.id === "verify" && (
          <div>
            <p className="payout-step-hint">Stripe will guide you through identity verification inside their dashboard. Once complete, your status will update here automatically (may take up to 24 hours).</p>
          </div>
        )}
        {current.id === "payout" && (
          <div>
            <p className="payout-step-hint">In your Stripe dashboard, go to <strong>Settings → Payouts</strong> and add a bank account or debit card.</p>
          </div>
        )}
        {current.id === "tax" && (
          <div>
            <p className="payout-step-hint">US authors: submit a W-9. International authors: submit a W-8BEN. These are required by law before we can release earnings. Stripe will collect this through their dashboard.</p>
          </div>
        )}
        {current.id === "confirm" && (
          <div>
            <label className="payout-confirm-label">
              <input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)} />
              <span>I have read and accept the <button className="pol-link-inline" onClick={() => go && go("payout-responsibility")}>Author Payout Account Responsibility terms</button>. I understand that Little Amour Books is not responsible for delayed or failed payments due to incomplete payout setup, and that my earnings may be held until my account is fully verified.</span>
            </label>
            <button className="payout-btn-gold" style={{ marginTop: 16 }} disabled={!confirmed} onClick={() => { markDone("confirm"); setDone(true); onComplete && onComplete({ ...checked, confirm: true }); }}>
        ...
        {current.id !== "confirm" && (
          <button className="payout-btn-gold" style={{ marginTop: 16 }} onClick={() => markDone(current.id)}>
            Mark as done — continue →
          </button>
        )}

        <button className="payout-skip" onClick={() => go && go("dashboard")}>
          Skip for now (earnings will be held until setup is complete)
        </button>
      </div>

      <div className="payout-policy-ref">
        <p>Your payout setup is governed by our <button className="pol-link-inline" onClick={() => go && go("payout-responsibility")}>Payout Account Responsibility Policy</button>. Held earnings do not accrue interest.</p>
      </div>
    </div>
  );
}
...
const PAYOUT_CSS = `
.payout-flow { max-width: 640px; margin: 0 auto; padding: 32px 24px; }
.payout-flow-header { margin-bottom: 20px; }
.payout-h2 { font-family: 'Fraunces', Georgia, serif; font-size: 26px; font-weight: 560; color: #131A30; margin-bottom: 8px; }
.payout-sub { font-size: 15px; color: #5E5468; line-height: 1.65; }
.payout-warning-banner { display: flex; gap: 12px; align-items: flex-start; background: #FFF6E8; border: 1.5px solid #F0D09A; border-radius: 12px; padding: 14px 16px; font-size: 14px; line-height: 1.6; color: #2B2433; }
.payout-warning-icon { font-size: 20px; flex-shrink: 0; }
.payout-steps { display: flex; gap: 0; overflow-x: auto; margin-bottom: 24px; padding-bottom: 4px; }
.payout-step-ind { display: flex; flex-direction: column; align-items: center; gap: 5px; flex: 1; min-width: 80px; opacity: .45; }
.payout-step-ind.on { opacity: 1; }
.payout-step-ind.done { opacity: .75; }
.payout-step-dot { width: 28px; height: 28px; border-radius: 50%; background: #ECD9C5; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 800; color: #6E5572; }
.payout-step-ind.on .payout-step-dot { background: #131A30; color: #fff; }
.payout-step-ind.done .payout-step-dot { background: #27ae60; color: #fff; }
.payout-step-lbl { font-size: 11px; text-align: center; color: #5E5468; font-weight: 600; }
.payout-step-card { background: #FAF4EB; border: 1.5px solid #ECD9C5; border-radius: 16px; padding: 28px 24px; margin-bottom: 20px; }
.payout-step-icon { font-size: 36px; margin-bottom: 10px; }
.payout-step-title { font-family: 'Fraunces', Georgia, serif; font-size: 20px; font-weight: 560; color: #131A30; margin-bottom: 10px; }
.payout-step-desc { font-size: 15px; color: #5E5468; line-height: 1.65; margin-bottom: 16px; }
.payout-step-hint { font-size: 13.5px; color: #888; line-height: 1.6; margin: 10px 0; }
.payout-btn-gold { display: inline-block; background: #E2A857; color: #fff; border: none; border-radius: 999px; padding: 12px 28px; font-size: 15px; font-weight: 700; cursor: pointer; text-decoration: none; }
.payout-btn-gold:disabled { opacity: .45; cursor: default; }
.payout-stripe-btn { display: inline-flex; align-items: center; gap: 8px; }
.payout-confirm-label { display: flex; gap: 10px; align-items: flex-start; font-size: 14px; line-height: 1.65; cursor: pointer; }
.payout-confirm-label input { flex-shrink: 0; margin-top: 3px; width: 16px; height: 16px; accent-color: #E2A857; }
.payout-skip { background: none; border: none; color: #aaa; font-size: 13px; cursor: pointer; margin-top: 14px; padding: 0; text-decoration: underline; display: block; }
.payout-policy-ref { font-size: 13px; color: #aaa; text-align: center; line-height: 1.6; }
.payout-done { text-align: center; padding: 48px 24px; }
`;
...
export { POLICY_VERSION, POLICY_CSS, PAYOUT_CSS };
